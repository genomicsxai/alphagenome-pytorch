...
- Enable in finetune.py with `--sequence-parallel [--overlap-highres N]`. There is no
  `--overlap-lowres` flag: the low-res overlap is computed as `overlap_highres // 128`.
  (The `SequenceParallelism` class itself does take an `overlap_lowres` argument.)
